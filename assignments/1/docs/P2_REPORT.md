...
| 7 | 写后读旁路（验证旁路行为） | 5 | 0 | 5 | ADD | rs1 = 3'b101; rs2 = 3'b000; rd = 3'b101; init_we = 1'b0; reg_we = 1'b1;（即读写同一寄存器 R5） | src_a = alu_result = 0x0008（见说明） | ✓ PASS |
...
- 注意：ALU 为组合逻辑，输出会在输入改变后立即反映；寄存器写入是同步在 `posedge clk`。波形上会先看到组合输出（`alu_result`/`src_a`）的变化，随后在时钟上升沿寄存器才被更新为相同的值。这种“先看到组合输出再看到寄存器写入”的现象是时序上的正常行为，不是错误。
...
- 写后读旁路：同周期读写时，读端口直接取 wdata，不等待寄存器写入
