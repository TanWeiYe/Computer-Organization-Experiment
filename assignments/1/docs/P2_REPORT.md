...
- 现象：在波形中，某些寄存器（如 `regs[6]`）会短暂显示 `0x0008`，随后变为 `0x0005`。这在 Step4 → Step7 的切换时段尤为明显（见上图）。

- 猜测（用户提出并经验证）：当写使能 `we` 持续为 1 且 `rd` 或其他控制信号在非对齐时刻改变时，写操作可能跨越多个时钟沿，导致上一次或瞬时的 ALU 输出被写入到后续目标寄存器，从而出现“先旧值后新值”的变化。

- 解决方法（已实现）：将写使能 `reg_we` 改为单周期脉冲 —— 在需要写入时，在 `@(posedge clk)` 处置位一次，然后立即清零。这样确保每次写操作只在单一上升沿发生，避免跨周期的意外写入。

- 后续实验计划：
   1. 使用单周期写脉冲后重新运行仿真并在 GTKWave 中截取关键时序（`R5`/`R6`/`alu_result`/`src_a`/`reg_we`/`rd`/`clk`）以验证问题消失。  
   2. 将 testbench 全面改为 `@(posedge clk)` 同步风格以提高时序可预测性。  
   3. 如仍出现边缘问题，考虑在写回路径中增加寄存器或引入短延迟以打破潜在的组合回路并评估对性能的影响。

（提示）我已在测试台实现了单周期写脉冲并运行了仿真；请您在本地重新运行并上传新的截图，我可以把最终截图替换到本报告中并提交最终版本。
